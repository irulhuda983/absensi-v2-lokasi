...
if (empty($_GET['building'])) {
      ...
      $building_id = mysqli_real_escape_string($connection,$_GET['building']);
}
...
if (empty($error)){
  ...
  if (($extension="jpg") && ($extension="jpeg") && ($extension="gif")) { 
    ...
    $query_building  ="SELECT radius from building WHERE building_id='$building_id'";
    $result_building = $connection->query($query_building);
    $row_building = $result_building->fetch_assoc();
      // Cek Radius Absensi ------------------------------
      if($row_building['radius'] > $radius){
        ...
        if($result->num_rows > 0){
          ...
          // Update Absensi Pulang
              if($row['time_out']=='00:00:00'){
                ...
                $directory= "../sw-content/present/".$filename;
                /* -------- Upload Foto Pulang -------*/
  ...
  if(strlen($file_name)){   
       ...
       if(in_array($ext,$valid)){   
         if($size<500000){   
           ...
           $photo_new   =''.$row_user['employees_id'].'-'.strip_tags(md5($file_name)).'-'.seo_title($time).'-'.$file_ext.'';
    ...
    if($result_absen->num_rows > 0){
      ...
      if($row_absen['status']=='Terlambat'){
        ...
        <tr>
            ...
            <td><a class="image-link" href="./sw-content/present/'.$row_absen['picture_in'].'">
            <span class="badge badge-success">'.$row_absen['time_in'].'</span></a></td>
            <td><a class="image-link" href="./sw-content/present/'.$row_absen['picture_out'].'">
